...
$htmlPath = Join-Path $outDirPath "freefire-hok-kira-price-comparison-$dateStamp.html"
$pdfPath = Join-Path $outDirPath "freefire-hok-kira-price-comparison-$dateStamp.pdf"
...
$allRows = Import-Csv -LiteralPath $csvFullPath |
    Where-Object { $_.Game -in @("Free Fire", "Honor of Kings") }

$sections = foreach ($game in @("Free Fire", "Honor of Kings")) {
...
<head>
  <meta charset="utf-8">
  <title>Free Fire and HOK Kira Price Comparison</title>
  <style>
...
<body>
  <header>
    <h1>Free Fire and Honor of Kings: Our Price vs Kira</h1>
